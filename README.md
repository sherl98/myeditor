...
仓库仅包含 Markdown 编辑器及其开发所需文件。个人文档、历史审计截图、旧对话记录、本机配置、依赖目录和构建产物不纳入版本控制。首次导入使用当前源码快照，不包含原开发目录的旧 Git 历史。

新增密钥或本机设置时使用被忽略的本地配置；不要将真实值写入源码或提交记录。第三方依赖许可证由 Web 构建脚本汇总并随应用资源附带。
